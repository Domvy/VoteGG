...
class OpenviduFinal extends Component {
    ...
    updateAudioStatus = () => {
        const { currentPhase, currentTurn, leftUserList, rightUserList, subscribers, session } = this.state;

        if (!session) {
            console.warn("세션이 초기화되지 않았습니다.");
            return;
        }

        const currentSpeaker =
            currentTurn === 'left'
                ? leftUserList[currentPhase - 1]
                : rightUserList[currentPhase - 1];

        if (!currentSpeaker) {
            console.warn("현재 발언자가 없습니다.");
            return;
        }

        // 발언자 외의 나머지 사용자들을 listener로 관리
        const listeners = subscribers.filter(
            (sub) => sub.connection.connectionId !== currentSpeaker.connectionId
        );

        // 발언자 설정
        if (this.state.publisher) {
            const isCurrentSpeaker =
                this.state.publisher.stream.connection.connectionId === currentSpeaker.connectionId;
            this.state.publisher.publishAudio(isCurrentSpeaker);
        }

        // listener 배열 내 사용자는 오디오 듣기 활성화
        listeners.forEach((listenerSub) => {
            if (listenerSub.subscriber && listenerSub.subscriber.stream) {
                listenerSub.subscriber.subscribeToAudio(true);
            }
        });

        // 발언자의 오디오는 발언자 본인에게만 허용
        const speakerSub = subscribers.find(
            (sub) => sub.connection.connectionId === currentSpeaker.connectionId
        );
        if (speakerSub?.subscriber) {
            speakerSub.subscriber.subscribeToAudio(false);
        }

        console.log("현재 발언자:", currentSpeaker.userName);
        console.log("현재 listener 리스트:", listeners.map((l) => l.userName));
    };
