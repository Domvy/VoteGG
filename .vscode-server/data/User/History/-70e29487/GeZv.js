...
class OpenviduFinal extends Component {
    ...
    updateAudioStatus = () => {
        const { currentPhase, currentTurn, leftUserList, rightUserList, session, subscribers, publisher } = this.state;
        ...
        if (!currentSpeaker || !currentSpeaker.connectionId) {
            console.warn("현재 발언자가 없거나 connectionId가 정의되지 않았습니다.");
            return;
        }
    
        console.log("현재 발언자:", currentSpeaker.userName);
    
        // listener 배열 생성 (발언자 제외)
        ...
        if (publisher) {
            const isCurrentSpeaker =
                publisher.stream?.connection?.connectionId === currentSpeaker.connectionId;
            publisher.publishAudio(isCurrentSpeaker);
        }
