...
class OpenviduFinal extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.isObserver) {
            this.joinListenerSession(); // 청취자는 ListenerSession에 참여
        } else {
            this.joinSession(); // 발언자는 기존 Session 참여
        }
    ...
    async joinListenerSession() {
        ...
        try {
            const token = await this.getToken(this.props.sessionId); // Token 생성
            await session.connect(token, { clientData: this.state.userName });

            this.setState({ session });
        } catch (error) {
            console.error('Error joining listener session:', error);
        }
    }
    ...
    updateAudioStatus = () => {
        const { currentPhase, currentTurn, leftUserList, rightUserList, session, subscribers } = this.state;
        ...
        // 현재 발언자 결정
        const currentLeftSpeaker = currentTurn === 'left' ? leftUserList[currentPhase - 1] : null;
        const currentRightSpeaker = currentTurn === 'right' ? rightUserList[currentPhase - 1] : null;

        // 발언자 오디오 활성화 여부
        const isLeftSpeaker =
            currentLeftSpeaker?.connectionId === localConnectionId;
        const isRightSpeaker =
            currentRightSpeaker?.connectionId === localConnectionId;

        const shouldEnableAudio = isLeftSpeaker || isRightSpeaker;
        ...
        subscribers.forEach((subscriber) => {
            const subscriberConnectionId = subscriber.subscriber.stream.connection.connectionId;

            // 발언자는 자신이 청취하지 않음
            if (
                subscriberConnectionId === currentLeftSpeaker?.connectionId ||
                subscriberConnectionId === currentRightSpeaker?.connectionId
            ) {
                subscriber.subscriber.subscribeToAudio(false);
            } else {
                subscriber.subscriber.subscribeToAudio(true);
            }
        });
    };
