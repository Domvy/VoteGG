...
const RoomList = () => {
  ...
  useEffect(() => {
    const fetchThumbnails = async () => {
      try {
        const response = await axios.get(window.location.origin + '/api/stream-thumbnails');
        const thumbnailData = response.data; // 서버에서 반환된 썸네일 목록

        // 방 번호와 매핑
        const thumbnailMap = {};
        rooms.forEach((room, index) => {
          thumbnailMap[room.roomNumber] = thumbnailData[index] || null;
        });

        setThumbnails(thumbnailMap);
      } catch (error) {
        console.error("썸네일 가져오기 실패:", error.message);
      }
    };
          ...
          <div key={room.roomNumber} className="room-card">
            {/* <div className="room-image">
             ...
             <div className="room-thumbnail">
              {thumbnails[room.roomNumber] ? (
                <img
                  src={thumbnails[room.roomNumber]}
