...
function timerSocketHandler(io) {
  ...
  timerNamespace.on('connection', (socket) => {
    ...
    socket.on('join_room', (roomId) => {
      ...
      if (!rooms[roomId]) {
        // 타이머 초기 설정
        rooms[roomId] = {
          durations: [4, 6], // 타이머 단계들의 지속 시간 (초)
      ...
      socket.on('phaseChange', (data) => {
        const { roomId, currentPhase, currentTurn } = data;
    
        // 잠금이 설정된 경우 중복 요청 무시
        if (phaseLocks[roomId]) {
          console.log(`PhaseChange 요청이 무시됨: 방 ${roomId}`);
          return;
        }
    
        // 잠금 설정
        phaseLocks[roomId] = true;
    
        console.log(`PhaseChange 요청 처리 중: 방 ${roomId}`);
    
        // 모든 클라이언트에 phaseChange 브로드캐스트
        timerNamespace.to(roomId).emit('phaseChange', { currentPhase, currentTurn });
    
        // 잠금 해제 (1초 후)
        setTimeout(() => {
          phaseLocks[roomId] = false;
        }, 1000); // 1초 동안 중복 방지
      });
  ...
  function moveToNextStage(roomId) {
    ...
    if (room.currentIndex >= room.durations.length) {
      ...
      // 모든 클라이언트에 phaseChange 이벤트 전송
      timerNamespace.to(roomId).emit('phaseChange', {
        currentPhase: room.currentCycle,
        currentTurn: room.currentIndex % 2 === 0 ? 'left' : 'right', // 예: 턴 순서 설정
      });
    }
