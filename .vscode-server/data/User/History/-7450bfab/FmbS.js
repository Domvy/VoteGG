...
class OpenviduFinal extends Component {
    ...
    async joinSession() {
        ...
        session.on('streamCreated', (event) => {
            ...
            const userName = connectionData.clientData || "Unknown";

            const newUser = {
                connectionId: event.stream.connection.connectionId,
                userName: userName,
                subscriber: subscriber, // 구독자 정보 포함
            };
    ...
    handleTurnChange = () => {
        ...
        if (currentTurn === 'odd') {
            ...
            newTurn = 'odd';
            newPhase = currentPhase === 1 ? 2 : 1; // Phase를 1과 2 사이에서 전환
        }
    ...
    updateAudioStatus = () => {
        const { usersList, currentPhase, currentTurn, leftUserList, rightUserList, session } = this.state;

        const activeUserIndex = (currentTurn === 'odd') ? (currentPhase * 2 - 2) : (currentPhase * 2 - 1);
        const activeUser = usersList[activeUserIndex];

        usersList.forEach((user, index) => {
            const shouldEnableAudio = index === activeUserIndex;

            if (user.connectionId === session.connection.connectionId) {
                // 현재 사용자
                if (this.state.publisher) {
                    this.state.publisher.publishAudio(shouldEnableAudio);
                    console.log(`현재 사용자(${user.connectionId}) 오디오 상태: ${shouldEnableAudio ? "활성화" : "비활성화"}`);
                }
            } else {
                // 다른 구독자
                user.subscriber.subscribeToAudio(shouldEnableAudio);
                console.log(`구독자(${user.connectionId}) 오디오 상태: ${shouldEnableAudio ? "활성화" : "비활성화"}`);
            }
        });
    };
