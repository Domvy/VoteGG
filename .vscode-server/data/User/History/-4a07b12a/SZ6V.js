...
app.post("/api/sessions/:sessionId/connections", async (req, res) => {
  ...
  if (!session) {
    ...
    const { userName } = req.body; // 클라이언트로부터 사용자 이름을 받습니다.

    // 해당 세션의 참가자 목록 초기화 또는 가져오기
    if (!sessionParticipants[session.sessionId]) {
      sessionParticipants[session.sessionId] = [];
    }
    const participants = sessionParticipants[session.sessionId];

    // 팀 배정 로직: 첫 번째와 두 번째 참가자는 'left', 이후 참가자는 'right'
    const team = participants.length < 2 ? 'left' : 'right';

    // 참가자 정보를 세션 참가자 목록에 추가
    participants.push({ userName, team });

    // connectionData에 사용자 이름과 팀 정보를 포함
    const connectionProperties = {
      data: JSON.stringify({ userName, team }),
      // 필요한 경우 다른 속성들도 포함 가능합니다.
    };
