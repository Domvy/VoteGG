...
app.post("/api/sessions/:sessionId/connections", async (req, res) => {
  ...
  if (!session) {
    ...
    const { userName } = req.body; // 클라이언트로부터 userName을 받음

    // userName이 없을 경우 에러 반환
    if (!userName) {
      return res.status(400).send({ error: 'userName is required' });
    }

    // 세션 참가자 목록 초기화 또는 가져오기
    ...
    // connectionData에 userName과 team 포함
    const connectionProperties = {
      data: JSON.stringify({ userName, team }),
    };

    // 로그로 데이터 확인
    console.log("Creating connection with data:", connectionProperties.data);
